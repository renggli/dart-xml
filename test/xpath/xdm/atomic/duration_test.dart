...
void main() {
  ...
  group('XPathDayTimeDuration', () {
    ...
    test('parsing tryParse', () {
      ...
      expect(XPathDayTimeDuration.tryParse('PT0.5S'), isNotNull);
      expect(XPathDayTimeDuration.tryParse('P1Y'), isNull);
      expect(XPathDayTimeDuration.tryParse('invalid'), isNull);
    });
    ...
    test('arithmetic operators', () {
      ...
      expect(-d1, equals(const XPathDayTimeDuration(-86400000000)));
      expect(const XPathDayTimeDuration(-86400000000).abs(), equals(d1));
      expect(d1 < d2, isTrue);
      expect(d2 > d1, isTrue);
      expect(d1 <= d1, isTrue);
      expect(d2 >= d2, isTrue);
    ...
    test('formatting stringValue', () {
      expect(const XPathDayTimeDuration(0).stringValue, equals('PT0S'));
      expect(
        const XPathDayTimeDuration(86400000000).stringValue,
        equals('P1D'),
      );
    });
  });
  ...
  group('XPathYearMonthDuration', () {
    test('construction and parsing', () {
      ...
      expect(XPathYearMonthDuration.tryParse('-P2Y'), isNotNull);
      expect(XPathYearMonthDuration.tryParse('P1D'), isNull);
      expect(XPathYearMonthDuration.tryParse('invalid'), isNull);
    });
    ...
    test('arithmetic operators', () {
      ...
      expect(-y1, equals(const XPathYearMonthDuration(-12)));
    ...
    test('formatting stringValue', () {
      expect(const XPathYearMonthDuration(0).stringValue, equals('P0M'));
      expect(const XPathYearMonthDuration(14).stringValue, equals('P1Y2M'));
      expect(const XPathYearMonthDuration(-14).stringValue, equals('-P1Y2M'));
    });
  });
}
