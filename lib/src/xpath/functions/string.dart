...
XPathValue string(XPathContext context, List<XPathValue> arguments) {
...
XPathValue concat(XPathContext context, List<XPathValue> arguments) {
  ...
  final result = arguments.map((each) => each.string);
  return XPathString(result.join());
}

// boolean starts-with(string, string)
XPathValue startsWith(XPathContext context, List<XPathValue> arguments) {
  XPathEvaluationException.checkArgumentCount('starts-with', arguments, 2);
  final string = arguments[0].string;
  final pattern = arguments[1].string;
  return XPathBoolean(string.startsWith(pattern));
}

// boolean contains(string, string)
XPathValue contains(XPathContext context, List<XPathValue> arguments) {
...
XPathValue substringBefore(XPathContext context, List<XPathValue> arguments) {
  XPathEvaluationException.checkArgumentCount('substring-before', arguments, 2);
  final string = arguments[0].string;
  final index = string.indexOf(arguments[1].string);
  return XPathString(index >= 0 ? string.substring(0, index) : '');
}

// string substring-after(string, string)
XPathValue substringAfter(XPathContext context, List<XPathValue> arguments) {
...
XPathValue substring(XPathContext context, List<XPathValue> arguments) {
  XPathEvaluationException.checkArgumentCount('substring', arguments, 2, 3);
  final string = arguments[0].string;
  final start_ = arguments[1].number;
  if (!start_.isFinite) return XPathString.empty;
  final start = start_.round() - 1;
  final end_ = arguments.length > 2 ? arguments[2].number : double.infinity;
...
XPathValue stringLength(XPathContext context, List<XPathValue> arguments) {
...
XPathValue normalizeSpace(XPathContext context, List<XPathValue> arguments) {
  XPathEvaluationException.checkArgumentCount(
    'normalize-space',
    arguments,
    0,
...
XPathValue translate(XPathContext context, List<XPathValue> arguments) {
  XPathEvaluationException.checkArgumentCount('translate', arguments, 3);
  final input = arguments[0].string;
  final sourceChars = arguments[1].string;
  final targetChars = arguments[2].string;
  final mapping = {
